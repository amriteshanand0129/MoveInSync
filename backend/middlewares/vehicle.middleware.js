...
const verifyVehicleRegistrationBody = async (req, res, next) => {
  try {
    if (req.user.vehicle) {
      return res.status(401).send({
        error: "Vehicle already registered for this driver. You cannot regsiter more than one vehicle",
      });
    }
    ...
    const existingVehicle = await model.findOne({ vehicle_number: req.body.vehicle_number });
    if (existingVehicle) {
